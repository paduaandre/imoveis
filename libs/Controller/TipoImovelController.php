	...
	public function Query()
	{
		try
		{
			...
			if ($page != '')
			{
				...
				$tipo = $this->Phreezer->Query('TipoImovel',$criteria)->GetDataPage($page, $pagesize);
				$output->rows = $tipo->ToObjectArray(true,$this->SimpleObjectParams());
				$output->totalResults = $tipo->TotalResults;
				$output->totalPages = $tipo->TotalPages;
				$output->pageSize = $tipo->PageSize;
				$output->currentPage = $tipo->CurrentPage;
			}
			else
			{
				// return all results
				$tipo = $this->Phreezer->Query('TipoImovel',$criteria);
				$output->rows = $tipo->ToObjectArray(true, $this->SimpleObjectParams());
